...
ll n,k,k2;
ll a[maxN],b[maxN],A[maxN],ac[maxN],Al[maxN],Ae[maxN],Be[maxN],ans[maxN],ans2[maxN];
int tax[maxN],pos,mm;
inline ll getint() {
	ll num = 0, bj = 1;
	char c = getchar();
	...
	while (c >= '0' && c <= '9') {
		num = (num * 10 + c - '0')%P, c = getchar();
	}
	return num * bj;
}
inline void getint(ll &a,ll& b) {
	ll bj = 1;a=b=0;
	char c = getchar();
	while (c < '0' || c > '9') {
		if (c == EOF)
...
{
	init();
	if(pos==-1||pos*k>=n||(pos>0&&mm>0)){
		for(int i=0;i<n;++i) cout<<0<<" ";cout<<endl;
		return 0;
	}
	proc();
	polyqp(n-pos,ac,k);
	for(int i=0;i<pos*k;++i) cout<<0<<" ";
	ll qk=qp(a[pos],k2);
	for(int i=0;i<n-pos*k;++i) cout<<ans2[i]*qk%P<<" ";cout<<endl;
}
